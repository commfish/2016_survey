...
rownames(worm.pn) <- c("0%", "1 - 24%", "25 - 49%", "50 - 74%", "75 - 100%", "n")
rownames(gonad.pn) <- c("Immature", "Empty", "Initial Recovery", "Filling", "Full", "Cannot Determine", "n")
rownames(blist.pn) <-c("0%", "1 - 24%", "25 - 49%", "50 - 74%", "n")   # no 4's (75-100%) this year
rownames(weak.pn) <- c("Good", "Weak", "n")

# reorder kayak beds together
worm.pn <- worm.pn[,c("EK1","WK1","KSH1", "KSH2", "KSH3")]
gonad.pn <- gonad.pn[,c("EK1","WK1","KSH1", "KSH2", "KSH3")]
blist.pn <- blist.pn[,c("EK1","WK1","KSH1", "KSH2", "KSH3")]
weak.pn <- weak.pn[,c("EK1","WK1","KSH1", "KSH2", "KSH3")]
