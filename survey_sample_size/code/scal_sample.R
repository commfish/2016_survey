...
replicate(10, kn, simplify = FALSE) %>%
   lapply(., samp_it, n=mean(kn$s5)) %>% 
   bind_rows %>%
   mutate(replicate = 1:n()) %>%
   group_by(SURVEY_AREA,BED,YEAR) %>% 
   summarize(Pbar = mean(P, na.rm=T), Psd=sd(P, na.rm=T)) -> kn5
...
replicate(1000, kw, simplify = FALSE) %>%
  ...
  mutate(replicate = 1:n()) %>%
  group_by(SURVEY_AREA,BED,YEAR) %>% 
   summarize(Pbar = mean(P,na.rm=T), Psd=sd(P,na.rm=T),Var=var(P,na.rm=T)) -> kw2
...
replicate(1000, kw, simplify = FALSE) %>%
  ...
  mutate(replicate = 1:n()) %>%
  group_by(SURVEY_AREA,BED,YEAR) %>% 
   summarize(Pbar = mean(P,na.rm=T), Psd=sd(P,na.rm=T),Var=var(P,na.rm=T)) -> kw3
#added a rep for 35 % of grids sampled
replicate(1000, kw, simplify = FALSE) %>%
  ...
  mutate(replicate = 1:n()) %>%
  group_by(SURVEY_AREA,BED,YEAR) %>% 
   summarize(Pbar = mean(P,na.rm=T), Psd=sd(P,na.rm=T),Var=var(P,na.rm=T)) -> kw35
...
replicate(1000, kw, simplify = FALSE) %>%
  ...
  mutate(replicate = 1:n()) %>%
  group_by(SURVEY_AREA,BED,YEAR) %>% 
   summarize(Pbar = mean(P,na.rm=T), Psd=sd(P,na.rm=T),Var=var(P,na.rm=T)) -> kw4
...
replicate(1000, kw, simplify = FALSE) %>%
  ...
  mutate(replicate = 1:n()) %>%
  group_by(SURVEY_AREA,BED,YEAR) %>% 
  summarize(Pbar = mean(P,na.rm=T), Psd=sd(P,na.rm=T),Var=var(P,na.rm=T)) -> kw5
...
kw5 %>% 
   ...
   summarize (Psd52.avg.per = mean(Psd52.dif.per, na.rm = T), 
              ...
              Psd535.avg.per = mean(Psd535.dif.per, na.rm = T), 
              Psd54.avg.per = mean(Psd54.dif.per, na.rm = T),
              P52 = mean(Pbar52.dif.per, na.rm = T),
              P53 = mean(Pbar53.dif.per, na.rm = T),
              P535 = mean(Pbar535.dif.per, na.rm = T),
              P54 = mean(Pbar54.dif.per, na.rm = T)) %>% 
   melt() %>% 
   mutate(sample_size = c(0.2,0.3,0.35,0.4,0.2,0.3,0.35,0.4),type=c(rep('sd',4),rep('mean',4)))-> kw5sum.sd
