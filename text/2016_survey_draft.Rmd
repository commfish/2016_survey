...
Survey design and implementation procedures are detailed in the statewide scallop survey operational plan [@Smith2016].  Sample collection is also detailed there. 

Scallop samples were analyzed for each survey area (bed) and categorized by size-class, large ($\geq$ 100 mm) and small (< 100 mm). 

Scallop abundance estimations were based upon area-swept calculations where scallop density $d_i$ is calculated as:
...
An estimate of the surveyed population round weight (lb) $\hat{RW}$ was calculated using the same formula as for the abundance estimates.  
...
MW_2 = \hat{RW}\bar r\
$$
Confidence intervals for the meat weight (lb) estimates were calculated from the nonparametric bootstraps performed for the abundance estimates (both $\hat{N}$ and $\hat{RW}$) and the mean ratio $\bar r$.  

A Guideline harvest level (GHL) was calculated based upon 5% and 10% pooulation exploitation levels.

Graphical representations of biological data are presented for evaluation.
...
Successful tows were performed in five beds during either May or July surveys (Figs 1 and 2). 
...
The meat weight to round weight ratio was between 0.08 and 0.12, with little variability found within beds (Table 4). Proposed GHLs were generally similar between the two meat weight estimates (Tables 5 & 6).
...
kable(worm, caption='Worms!')
```

```{r}
gonad <- read_csv('../output/gonad.csv')

kable(gonad, caption='Gonads!')
```

```{r}
blist <- read_csv('../output/blist.csv')

kable(blist, caption='Blisters!')
```

```{r}
weak <- read_csv('../output/weak.csv')

kable(weak, caption='Weak meats!')
```
...
![Scallop shell height histograms by bed. Sampled shell heights have been expanded to the number sampled. Note the differing y-axis.](../figs/Heights.png)