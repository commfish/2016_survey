...
The abundance (by weight) of clappers (empty shells) per bed was calculated using the same procedure as live scallops.  The clapper abundnace by bed was then used to estimate the percentage of clappers by bed.  This value may be helpful to understand natural mortality in each survey area. 
...
###Clappers
The abundance (by weight) of clappers was calculated by bed.  It was then evaluated, along with the abundance (by weight) of live scallops to estimate the percentage of clappers by bed (Table 6).  These estimates are higher than expected and further review of the assumptions and methods behind this analysis is recommended.   
...
**Table 1:** Coefficient of variation (CV) for abundance of large scallops by bed, also sample size and area by bed
``````{r, echo=F, message=F, warning=F}
a <- read.csv('../output/bed_sum_table_Ndbar.csv')
...
a %>% mutate(N=round(N_b),N_UL=round(ulN),N_LL=round(llN),dbar=round(dbar_b),dbar_UL=round(uldbar),dbar_LL=round(lldbar), size.class = variable) %>% select(Bed, size.class, N,N_UL,N_LL,dbar,dbar_UL,dbar_LL) %>%  mutate_each(funs(prettyNum(., big.mark=","))) %>%  mutate_each(funs(prettyNum(., big.mark=","))) %>%  kable()
...
f %>% filter(variable == 'large') %>% mutate(RW=round(N_lb),RW_UL=round(ulN),RW_LL=round(llN), size.class = variable) %>% select(Bed, year, size.class, RW, RW_UL, RW_LL) %>%  mutate_each(funs(prettyNum(., big.mark=","))) %>% kable()

```


###Meat Weight - large scallops 
...
d %>% mutate(N=round(N_b),N_UL=round(ulN),N_LL=round(llN),ratio =round(ratio_bar, 3), ratio_LL=round(ll, 3),ratio_UL=round(ul,3)) %>% select(Bed, N,N_UL,N_LL,weight, ratio, min_meat_wt, meat_wt, max_meat_wt, meat_0.05) %>%  mutate_each(funs(prettyNum(., big.mark=","))) %>% kable()
```
...
e %>% mutate(W_lb=round(N_lb),W_UL=round(ulN),W_LL=round(llN),ratio =round(ratio_bar, 3), ratio_LL=round(ll, 3),ratio_UL=round(ul,3)) %>% select(Bed, W_lb,W_UL,W_LL,ratio, min_meat_wt, meat_wt, max_meat_wt, meat_0.05) %>%  mutate_each(funs(prettyNum(., big.mark=","))) %>% kable()
```
