...
library(knitr)
b <- read.csv('../output/CV_table_Ndbar.csv')
library(tidyverse)
options(scipen=9999) # remove scientific notation
b %>% 
  mutate(dbar = round(dbar), cv = round(cv), N = round(N), cvN = round(cvN)) %>%
  select(Bed, variable, n, area, dbar, cv, N, cvN) %>% kable()
```
...
library(tidyverse)
options(scipen=9999) # remove scientific notation
a %>% mutate(N=round(N),N_UL=round(ulN),N_LL=round(llN),dbar=round(dbar),dbar_UL=round(uldbar),dbar_LL=round(lldbar)) %>% select(Bed, variable,N,N_UL,N_LL,dbar,dbar_UL,dbar_LL) %>% kable()
